...
Import `createHexClient` from `@crazycatviking/hex` and bundle the app for the browser. Hex does not install dependencies or run the build. Configure a relative asset base (for example Vite `base: './'`). Publishing defaults to `dist`; set `hex.json.directory` only for a different output directory such as `public`. Do not publish source directories, secrets or node_modules.
...
Add optional `title`, `description`, and `author` strings to hex.json for site attribution. Ask the user for author information rather than inventing it. The CLI publishes only these fields and a generated UTC `publishedAt` timestamp in `.hex-site.json`; connection settings and local paths are excluded. Source configuration and build output are not modified. Attribution is descriptive, not verified identity or access control.
