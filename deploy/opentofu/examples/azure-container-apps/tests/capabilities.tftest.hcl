mock_provider "azapi" {
  ...
  mock_data "azapi_resource_action" {
    defaults = {
      sensitive_output = {
        keys = [{ value = "test-only-storage-key" }]
      }
    }
  }
...
run "sites_only" {
  ...
  assert {
    condition = (
      length(module.sites) == 1 &&
      length(module.files) == 0 &&
...
run "missing_external_database_url" {
  ...
  variables {
    capabilities = { database = "external" }
  }
