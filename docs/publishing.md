...
The operator must authorize publishers with **Storage File Data Privileged Contributor** at an appropriate Azure Files scope. Hex checks the Azure CLI session for an Azure Storage token. If sign-in is needed, it runs `az login --allow-no-subscriptions --scope https://storage.azure.com/.default --output none`, with an optional tenant. Azure CLI opens the Entra authorization-code browser flow, supporting MFA and the organization's Conditional Access policies. Hex disables the Windows broker for these child processes to select the requested web flow, and disables the subscription picker. These settings do not change the user's global Azure CLI configuration.

AzCopy receives `AZCOPY_AUTO_LOGIN_TYPE=AZCLI` and obtains tokens from Azure CLI, including refreshes. Hex does not copy or store tokens or invoke AzCopy's device-code login. `hex login` explicitly refreshes the Azure CLI browser login when requested. Install Azure CLI using `winget install --exact --id Microsoft.AzureCLI` on Windows, or the [Microsoft installation instructions](https://learn.microsoft.com/cli/azure/install-azure-cli) for other operating systems.

Non-interactive commands report when browser sign-in is required. Linux sign-in needs a desktop session with DISPLAY set; headless sessions and Codespaces are rejected rather than deliberately choosing device-code authentication. `AZCOPY_AUTO_LOGIN_TYPE=DEVICE` is rejected. Automation can still supply `HEX_PUBLISH_SAS` or an explicit non-device mode such as SPN/MSI/WORKLOAD. `AZCOPY_TENANT_ID` selects the tenant for both browser login and the storage-token check. Progress goes to stderr; successful publish stdout remains the site URL.
